    ...
    public class EnhancedHierarchy
    {
        ...
        private static string GetChildCount(GameObject gameObject)
        {
            if (!GameObjectToChildCount.TryGetValue(gameObject, out var childCount))
            {
                ...
                gameObject.GetComponentsInChildren(false, Transforms);
                int activePercent = 100;
                if (count > 0)
                    activePercent = (Transforms.Count - 1) * 100 / count;
                Transforms.Clear();
                
                if (count == 0)
                    childCount = null;
                else if (count < 1000)
                    childCount = $"{count}, {activePercent}%";
                else if (count < 1000000)
                    childCount = $"{count / 1000}.{count % 1000 / 100}K, {activePercent}%";
                else
                    childCount = $"{count / 1000000}.{count % 1000000 / 100000}M, {activePercent}%";
